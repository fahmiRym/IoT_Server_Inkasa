    ...
    <style>
        ...
        * { cursor: crosshair; }
        ...
        .node-status-indicator {
            ...
            background: rgba(255, 0, 85, 0.1); overflow: hidden;
        }
        ...
        @keyframes breath { 0%, 100% { transform: scale(0.8); opacity:0.5; } 50% { transform: scale(1.1); opacity:1; } }
        ...
        .glass-card:hover { border-color: var(--cyan); transform: translateX(5px); }
        ...
        .matrix-item { text-align: center; border: 1px solid rgba(255,255,255,0.05); padding: 5px; border-radius: 6px; }
        .matrix-val { font-family: 'JetBrains Mono'; font-size: 1rem; color: var(--cyan); }
        .matrix-lab { font-size: 0.55rem; color: var(--text-dim); text-transform: uppercase; letter-spacing: 1px; }

        /* --- METRICS --- */
        main { display: flex; flex-direction: column; gap: 15px; min-width: 0; }
        .metrics-hud { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; }
        ...
        .val-large { font-family: 'Orbitron'; font-size: 2.2rem; font-weight: 900; line-height: 1; margin: 10px 0; }
        .unit { font-size: 1rem; color: var(--text-dim); margin-left: 5px; }
        .stat-footer { display: flex; justify-content: space-between; border-top: 1px solid var(--border); padding-top: 10px; font-size: 0.8rem; margin-top: auto; }
        ...
        /* --- CHARTS --- */
        .charts-row { display: none; } /* Hidden as requested */
        ...
        .cyber-btn:hover { background: var(--cyan); color: #000; box-shadow: 0 0 15px var(--cyan); }
        .cyber-btn.pdf { border-color: var(--pink); color: var(--pink); }
        .cyber-btn.pdf:hover { background: var(--pink); color: #fff; box-shadow: 0 0 15px var(--pink); }
        ...
        th { background: rgba(0, 243, 255, 0.05); padding: 12px; text-align: left; font-size: 0.75rem; color: var(--cyan); }
        td { padding: 12px; border-bottom: 1px solid var(--border); font-family: 'JetBrains Mono'; font-size: 0.85rem; }
        .row-danger { background: rgba(255, 0, 85, 0.1); }
        ...
        @keyframes flash-red {
            from { background: rgba(255, 0, 85, 0.4); }
        ...
        .alert-box h2 { font-family: 'Orbitron'; color: var(--pink); font-size: 3rem; margin: 0; }
        .alert-box p { font-size: 1.5rem; margin: 20px 0; }
        ...
        .badge-secure { background: rgba(10, 255, 0, 0.2); color: var(--green); border: 1px solid var(--green); }
        .badge-danger { background: rgba(255, 0, 85, 0.2); color: var(--pink); border: 1px solid var(--pink); }

        /* --- ANIMATIONS --- */
        .glow-text { text-shadow: 0 0 10px currentColor; }
        .moving-grid { animation: slide 20s linear infinite; }
        @keyframes slide { from { background-position: 0 0; } to { background-position: 50px 50px; } }

        .sub-val { font-size: 0.65rem; color: var(--text-dim); font-family: 'JetBrains Mono'; }
        .node-badge { 
            display: inline-block; padding: 1px 4px; border-radius: 3px; font-size: 0.6rem; 
            background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);
            margin-right: 2px;
        }
        .node-badge.active { color: var(--pink); border-color: var(--pink); background: rgba(255,0,85,0.1); }
        ...
        input:focus, select:focus { border-color: var(--cyan); box-shadow: 0 0 10px var(--accent-glow); }
        ...
        .label-caps { font-size: 0.6rem; color: var(--text-dim); text-transform: uppercase; letter-spacing: 2px; font-weight: 700; }
...
<div id="alert-overlay">
    <div class="alert-box">
        <i class="fas fa-exclamation-triangle" style="font-size: 5rem; color: var(--pink); margin-bottom: 20px;"></i>
        <h2>SYSTEM CRITICAL</h2>
        <p id="alert-msg">HIGH VALUES DETECTED</p>
        <button onclick="dismissAlert()" class="cyber-btn" style="border-color: var(--pink); color: var(--pink); width: 200px; margin: 0 auto;">DISMISS ALARM</button>
    </div>
</div>
...
<div class="container">
    <header>
        <div class="brand-container">
            ...
            <div class="brand">
                ...
                <div style="color: var(--cyan); font-family: 'JetBrains Mono'; font-size: 0.7rem; opacity: 0.6; display: flex; gap: 10px;">
    ...
    <aside>
        <div class="glass-card">
            ...
            <div id="status-hero" style="display: flex; gap: 15px; margin-bottom: 20px; align-items: center;">
                ...
                <div>
                    <div id="conn-text" style="font-weight: 900; font-size: 1.2rem; letter-spacing: 2px; color: var(--green); font-family: 'Orbitron';">CORE ACTIVE</div>
                    <div style="display: flex; gap: 10px; margin-top: 4px;">
                        <span class="sub-val">UPTIME: <span id="uptime-val" style="color:#fff">--:--:--</span></span>
            ...
            <div class="health-matrix">
                ...
                <div class="matrix-item">
                    <div class="matrix-val" style="color:var(--yellow); font-size: 0.8rem;" id="last-fetch">--:--:--</div>
                    <div class="matrix-lab">LAST_FETCH</div>
                </div>
                <div class="matrix-item" style="grid-column: span 2; border-color: var(--yellow);">
                    <div class="matrix-val" style="font-size: 0.75rem; color: var(--yellow); letter-spacing: 1px;">PACK_V2_ACTIVE</div>
        ...
        <div class="glass-card">
            ...
            <form id="reportForm" action="{{ route('export.csv') }}" method="GET">
                <div style="margin-bottom: 15px;">
                    <label class="label-caps"><i class="fas fa-calendar-alt" style="margin-right:5px"></i> DATE RANGE SELECTION</label>
                ...
                <div style="margin-bottom: 15px;">
                    <label class="label-caps"><i class="fas fa-layer-group" style="margin-right:5px"></i> MONTH SELECTION</label>
                    <div style="margin-top: 8px;">
                        <input type="month" name="month">
                    </div>
                </div>
                <div style="margin-bottom: 20px;">
                    <label class="label-caps"><i class="fas fa-archive" style="margin-right:5px"></i> YEARLY ARCHIVE</label>
                    <div style="margin-top: 8px;">
                        <select name="year">
        ...
        <div class="glass-card" style="border-bottom: 2px solid var(--border);">
            ...
            <button onclick="toggleMute()" class="cyber-btn" style="width: 100%; justify-content: space-between; padding: 15px; background: rgba(0, 243, 255, 0.03);">
                <div style="display:flex; align-items:center; gap:10px;">
                    <i class="fas fa-shield-alt" id="bell-icon" style="color:var(--cyan); filter:drop-shadow(0 0 5px var(--cyan))"></i>
                    <span style="font-family:'Orbitron'; font-size:0.8rem">ALARM_MODE</span>
                </div>
                <span id="alarm-status" style="font-family:'Orbitron'; color:var(--cyan); letter-spacing:1px;">ACTIVE</span>
            </button>
            <div id="safety-log" style="font-size: 0.65rem; color: var(--text-dim); margin-top: 15px; font-family: 'JetBrains Mono'; padding: 10px; background: rgba(0,0,0,0.3); border-radius: 6px; border-left: 2px solid var(--text-dim);">
    ...
    <main>
        <div class="metrics-hud">
            ...
            <div class="metric-glow" style="border-left: 4px solid var(--cyan);">
                <div style="display:flex; justify-content:space-between; align-items:start;">
                    ...
                    <i class="fas fa-thermometer-half" style="color:var(--cyan); font-size:1.2rem; filter:drop-shadow(0 0 5px var(--cyan))"></i>
                </div>
                <div class="val-large" id="t-val">--<span class="unit">°C</span></div>
                <div id="t-limit-text" style="font-size: 0.65rem; color: var(--cyan); margin-bottom: 5px; font-family: 'JetBrains Mono'; opacity: 0.8; height: 12px;">
                    <!-- AUTO POPULATED -->
                </div>
            ...
            <div class="metric-glow" style="border-left: 4px solid var(--yellow);">
                <div style="display:flex; justify-content:space-between; align-items:start;">
                    ...
                    <i class="fas fa-droplet" style="color:var(--yellow); font-size:1.2rem; filter:drop-shadow(0 0 5px var(--yellow))"></i>
                </div>
            ...
            <div class="metric-glow" id="card-smoke" style="border-left: 4px solid var(--green);">
                <div style="display:flex; justify-content:space-between; align-items:start;">
                    ...
                    <i class="fas fa-wind" style="color:var(--green); font-size:1.2rem; filter:drop-shadow(0 0 5px var(--green))"></i>
                </div>
                <div class="val-large" id="s-val">----<span class="unit">PPM</span></div>
                <div style="display: flex; gap: 5px; margin-top: -5px; margin-bottom: 5px; font-size: 0.6rem; font-family: 'JetBrains Mono'; opacity: 0.7;">
                    <div style="flex:1; border: 1px solid var(--border); padding: 2px; text-align:center;">S1:<span id="s1-val">--</span></div>
                    <div style="flex:1; border: 1px solid var(--border); padding: 2px; text-align:center;">S2:<span id="s2-val">--</span></div>
                    <div style="flex:1; border: 1px solid var(--border); padding: 2px; text-align:center;">S3:<span id="s3-val">--</span></div>
                </div>
                <div id="s-limit-text" style="font-size: 0.65rem; color: var(--green); margin-bottom: 5px; font-family: 'JetBrains Mono'; opacity: 0.8; height: 12px;">
                    <!-- AUTO POPULATED -->
                </div>
                <div class="mini-chart-box">
            ...
            <div class="metric-glow" id="card-fire" style="border-left: 4px solid var(--text-dim);">
                ...
                <div style="display: flex; gap: 5px; margin-top: -5px; margin-bottom: 5px; font-size: 0.6rem; font-family: 'JetBrains Mono'; opacity: 0.7;">
                    <div style="flex:1; border: 1px solid var(--border); padding: 2px; text-align:center;">F1:<span id="f1-val">OK</span></div>
                    <div style="flex:1; border: 1px solid var(--border); padding: 2px; text-align:center;">F2:<span id="f2-val">OK</span></div>
                </div>
                <div class="mini-chart-box">
        ...
        <!-- MASTER LOG STREAM (DIRECTLY BELOW CARDS) -->
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>HUM_VAL</th>
                            <th>AQS_PPM</th>
                            <th>SENSOR_BREAKDOWN (S1,S2,S3 | F1,F2)</th>
                            <th>SYSTEM_STATUS</th>
                        </tr>
                    </thead>
                    <tbody id="logs-table-body">
                        <tr><td colspan="6" style="text-align:center; padding: 30px; color: var(--text-dim);">PENDING CORE SYNC...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
<script>
    ...
    function updateCoreLogs(recent) {
        ...
        tbody.innerHTML = recent.map(log => {
            ...
            const smokeBreakdown = `
                <span class="node-badge">S1:${log.smoke1 || 0}</span>
                <span class="node-badge">S2:${log.smoke2 || 0}</span>
                <span class="node-badge">S3:${log.smoke3 || 0}</span>
            `;
            
            const flameBreakdown = `
                <span class="node-badge ${log.flame1 ? 'active' : ''}">F1:${log.flame1 ? '🔥' : 'OK'}</span>
                <span class="node-badge ${log.flame2 ? 'active' : ''}">F2:${log.flame2 ? '🔥' : 'OK'}</span>
            `;
                ...
                <tr class="${danger ? 'row-danger' : ''}">
                    ...
                    <td>
                        <div style="display:flex; flex-direction:column; gap:4px;">
                            <div>${smokeBreakdown}</div>
                            <div>${flameBreakdown}</div>
                        </div>
                    </td>
                    <td>
